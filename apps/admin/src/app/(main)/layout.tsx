...
import { getUserProfile } from "@/actions/auth/user";
import { verifySession } from "@/lib/session";
import { AuthProvider } from "@/providers/auth-provider";
import ErrorMessage from "@repo/ui/components/error-card";
import { Separator } from "@repo/ui/components/ui-kit/separator";
import {
  SidebarInset,
  SidebarProvider,
  SidebarTrigger,
...
export default async function Layout({
  ...
  const { userId, roles, permissions } = await verifySession();
  ...
  if (!userResponse?.success) {
    return (
      <ErrorMessage
        error={{
          type: userResponse?.error?.type || "SERVER_ERROR",
          message: "",
          details: {},
        }}
      />
