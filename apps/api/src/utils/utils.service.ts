...
export class UtilsService {
  ...
  public async removeFileFromLocal(filePathStr: string): Promise<boolean> {
    ...
    // Try deleting again if it fails once
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        await fs.unlink(filePath);
        this.logger.log(`File deleted successfully (attempt ${attempt}): ${filePathStr}`);
        return true;
      } catch (err) {
        if (err.code === 'ENOENT') {
          this.logger.warn(`File not found: ${filePathStr}`);
          return false; // No need to retry — file already missing
        }
        this.logger.warn(`Attempt ${attempt} failed to delete file: ${filePathStr}`);
        if (attempt === 3) {
          this.logger.error(`File deletion failed after 3 attempts: ${filePathStr}`);
          return false;
        }
      }
    }
